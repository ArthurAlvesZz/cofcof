...
export interface AdminDrawerProps {
  ...
  variant?: 'default' | 'danger' | 'operation' | 'dark' | 'light';
...
export function AdminDrawer({ 
  ...
  const canCloseOutside = !preventBackdropClose && !isDirty && !isBusy;
    ...
    switch (variant) {
      ...
      case 'light':
        return {
          ...
          closeBtn: 'text-gray-400 hover:text-[#111111] hover:bg-gray-100',
          bodyBg: 'bg-[#fcfaf8]',
          eyebrowColor: 'text-[#c9a263]'
        };
      default:
    ...
    <AnimatePresence>
      {isOpen && (
        <div 
          ...
          aria-describedby={subtitle ? "admin-drawer-subtitle" : undefined}
        >
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <div className="fixed inset-y-0 right-0 flex max-w-full pl-10 sm:pl-16">
            ...
            >
              <div className={`flex h-full flex-col shadow-2xl border-l ${vStyles.border} ${vStyles.bodyBg}`}>
                {/* Header */}
                <div className={`flex items-start justify-between px-6 py-5 border-b ${vStyles.border} ${vStyles.headerBg} shrink-0`}>
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <h2 id="admin-drawer-title" className={`text-xl sm:text-2xl font-serif ${vStyles.titleColor} truncate`}>{title}</h2>
                      {statusBadge && <div className="shrink-0">{statusBadge}</div>}
                    </div>
                    {subtitle && <p id="admin-drawer-subtitle" className={`text-sm mt-1.5 line-clamp-2 ${vStyles.subtitleColor}`}>{subtitle}</p>}
                  </div>
                ...
                {/* Body */}
                <div className={`relative flex-1 px-4 sm:px-6 py-6 overflow-y-auto custom-scrollbar ${vStyles.bodyBg}`}>
                  {children}
                </div>
                
                {/* Footer */}
                {generatedFooter && (
                  <div className={`flex-shrink-0 border-t flex items-center justify-between px-6 py-4 ${variant === 'dark' || variant === 'operation' ? 'border-[#333] bg-[#0a0a0a]' : 'border-[#a3a3a3]/10 bg-white'}`}>
                    {generatedFooter}
                  </div>
                )}
              </div>
