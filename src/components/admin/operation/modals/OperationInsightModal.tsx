...
export function OperationInsightModal({ isOpen, onClose, type, dates, stats, onAction }: InsightModalProps) {
  ...
  const contentMap: Record<string, any> = {
    ...
    'b2b_module': { title: 'Acesso B2B Leads', icon: Users, color: 'text-purple-500', bg: 'bg-purple-50', emptyTitle: 'Acessar módulo externo', emptyDesc: 'Por favor, utilize o menu lateral principal (Sidebar) para acessar a gestão completa de Parceiros B2B e Leads. Esta visão está fora da alçada diária de chão-de-fábrica.', emptyBtn: null },
    'product_module': { title: 'Acesso Produtos', icon: Package, color: 'text-indigo-500', bg: 'bg-indigo-50', emptyTitle: 'Acessar módulo externo', emptyDesc: 'Por favor, utilize o menu lateral principal (Sidebar) para acessar o catálogo de Produtos e E-commerce.', emptyBtn: null },
    'partner_module': { title: 'Acesso Parceiros e Lojas', icon: Target, color: 'text-emerald-500', bg: 'bg-emerald-50', emptyTitle: 'Acessar módulo externo', emptyDesc: 'Para validar rotas e coordenadas geográficas de parceiros, utilize o menu principal de Parceiros.', emptyBtn: null },
    'order_module': { title: 'Gestão de Pedidos', icon: Package, color: 'text-blue-500', bg: 'bg-blue-50', emptyTitle: 'Acessar módulo externo', emptyDesc: 'Para despachar pedidos de E-commerce pendentes, navegue até a aba Pedidos no menu lateral.', emptyBtn: null }
  };
                                               ...
                                               <div className="mt-2">
                                                   ...
                                                   {type === 'new_customers' && (
                                                       <div className="flex flex-col gap-2">
                                                           <p className="text-sm font-black text-[#1C1A17]">{item.name || item.companyName} <span className="text-gray-400 font-medium whitespace-nowrap">({item.type === 'b2b' ? 'B2B/Atacado' : 'B2C/Varejo'})</span></p>
                                                           {item.document && <p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest leading-none">Doc: <span className="text-[#1C1A17]">{item.document}</span></p>}
                                                           {(item.email || item.phone) && (
                                                              <p className="text-[11px] font-medium text-gray-500 leading-none">Contato: {item.email} {item.phone && `| ${item.phone}`}</p>
                                                           )}
                                                       </div>
                                                   )}
